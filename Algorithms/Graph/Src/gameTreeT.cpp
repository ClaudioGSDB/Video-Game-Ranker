...
#include "gameTreeT.h"
gameTreeT::treeNode::treeNode(bool leaf, bool child) : isLeaf(leaf), isChild(child){}


gameTreeT::gameTreeT() = default;

// given a game, and a parent node (root by default)
bool gameTreeT::insert(DataNodeT &game, treeNode* parent, bool correct) {
	...
	if(!correct){
		...
		pair<int, DataNodeT> middle;
		vector<pair<int, DataNodeT>> del;
		int middleI  = 0;
...
int gameTreeT::leafAmount() {
	return count;
}
void gameTreeT::findMatch(map<int, string> const &genres, vector<DataNodeT> &ret, gameTreeT::treeNode *parent) {
	if(!parent){
		parent = root;
	}
