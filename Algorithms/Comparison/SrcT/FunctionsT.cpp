...
void mainT()//Not in use for final submission
{
    //input - put in function
    map<int, string> inputGenMap = genrePickT();
    ...
    vector<DataNodeT> outputGames = findTopT(inputGenMap, genreMap);
...
vector<DataNodeT> findTopT(map<int, string> genres, map<int, gameTreeT*> &genreMap)
{
...
map<int, string> genrePickT() //for inputting the genres
{
